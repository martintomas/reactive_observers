  ...
  module ObservableServices
    class RemovingTest < ActiveSupport::TestCase
      ...
      test '#perform - remove observer for defined klass' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers, Observer, {}).perform
      ...
      test '#perform - remove observer for defined klass and specific record' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers, Observer, constrain: [Topic.first.id]).perform
      ...
      test '#perform - remove observer for defined object' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers, @observed_object, {}).perform
      ...
      test '#perform - remove observer for defined object and specific record' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers, @observed_object, constrain: [Topic.first.id]).perform
      ...
      test '#perform - remove observer for defined klass with additional arguments' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, Observer, on: :create).perform
        assert_equal 6, result.length
        assert result.none?(@observer1)
        assert result.none?(@observer2)
        assert result.none?(@observer3)
        ...
        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, Observer, on: :destroy).perform
        assert_equal 8, result.length
        assert result.none?(@observer1)
        assert result.none?(@observer3)

        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, Observer, on: :destroy, constrain: [Topic.first.id]).perform
        assert_equal 9, result.length
        assert result.none?(@observer3)
      end

      test '#perform - remove observer for defined object with additional arguments' do
        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, @observed_object, on: :create).perform
        assert_equal 6, result.length
        assert result.none?(@observer6)
        assert result.none?(@observer7)
        assert result.none?(@observer8)
        ...
        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, @observed_object, on: :destroy).perform
        assert_equal 8, result.length
        assert result.none?(@observer6)
        assert result.none?(@observer8)

        result = ReactiveObservers::ObservableServices::Removing.new(@observers.dup, @observed_object, on: :destroy, constrain: [Topic.first.id]).perform
        assert_equal 9, result.length
        assert result.none?(@observer8)
      end
    end
